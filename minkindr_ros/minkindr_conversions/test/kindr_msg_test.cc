...
TEST(KindrMsgTest, poseKindr2DToMsgToKindr2D) {
  for (auto const angle_rad : {-0.5, 0.5}) {
    ...
    EXPECT_NEAR(output_transform.getRotation().angle(), angle_rad,
                kTestTolerance)
        << "Angle: " << angle_rad;
    EXPECT_NEAR_EIGEN(output_transform.getPosition(), position, kTestTolerance);
  }
...
TEST(KindrMsgTest, poseMsgToKindr2DFailsForInvalidInputPose) {
  ...
  EXPECT_DEATH(poseMsgToKindr2D(invalid_position_msg,
                                &invalid_position_output_transform),
  ...
  EXPECT_DEATH(poseMsgToKindr2D(invalid_rotation_msg,
                                &invalid_rotation_output_transform),
...
TEST(KindrMsgTest, quaternionKindrToMsgToKindr) {
  ...
  EXPECT_NEAR_EIGEN(output_rotation.coeffs(), rotation.coeffs(),
                    kTestTolerance);
}
